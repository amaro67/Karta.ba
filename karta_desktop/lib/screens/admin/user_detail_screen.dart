...
class _UserDetailScreenState extends State<UserDetailScreen> {
  ...
  void initState() {
    ...
    // Set initial edit mode if specified
  ...
  Future<void> _handleSave() async {
    ...
    if (widget.isOwnProfile) {
      // Use AuthProvider for own profile
      ...
      // Use AdminProvider for admin view
      final adminProvider = Provider.of<AdminProvider>(context, listen: false);
      final authProvider = Provider.of<AuthProvider>(context, listen: false);
      ...
      success = await adminProvider.updateUser(
        ...
        lastName: _lastNameController.text.trim().isNotEmpty 
            ? _lastNameController.text.trim() 
            : null,
        email: null, // Email se nikada ne može mijenjati
      ...
      // Reload user orders after successful update to prevent them from disappearing
      if (success) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            adminProvider.loadUserOrders(_userId);
  ...
  Future<void> _handleRoleToggle(String role, bool value) async {
    // Role management is only for admin view, not for own profile
    if (widget.isOwnProfile) {
      return;
    }
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  // Perforated divider
                  _PerforatedDivider(),
                  // Body with max width constraint
                  Center(
    ...
    return Column(
      ...
      children: [
        ...
        _TicketField(
          ...
          enabled: false, // Email se nikada ne može mijenjati
        ...
        // Email Confirmed Checkbox (only for admin view, not for own profile)
        if (!isOwnProfile)
          CheckboxListTile(
        ...
        // Role Management Section (only for admin view, not for own profile)
        if (!isOwnProfile) ...[
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
...
class _UserOrdersListState extends State<_UserOrdersList> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    super.initState();
    // Load orders when widget is first created
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        final adminProvider = Provider.of<AdminProvider>(context, listen: false);
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<AdminProvider>(
      builder: (context, adminProvider, child) {
        // Cache orders when they're loaded from provider
        if (adminProvider.userOrders.isNotEmpty) {
          _cachedOrders = List<dynamic>.from(adminProvider.userOrders);
        }

        // Use cached orders if available, otherwise use provider orders
        // This prevents orders from disappearing during rebuild
