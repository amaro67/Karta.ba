...
class _TicketDetailScreenState extends State<TicketDetailScreen> {
  ...
  void _initializeData() {
    ...
    // Check if we received pre-loaded data from ticket list screen
    ...
    // If we have pre-loaded data, use it immediately without any loading
    if (preloadedEvent != null && preloadedOrderItem != null) {
      // Set data directly - no loading state needed
      setState(() {
    ...
    // Otherwise, load data (fallback for direct navigation)
    setState(() {
      _isLoading = true;
      _error = null;
    });
  ...
  Future<void> _loadTicketData() async {
    ...
    try {
      ...
      // If we have event but not order item, just load order item
      if (preloadedEvent != null && preloadedOrderItem == null) {
      ...
      // Fetch orders to find the order item for this ticket
      final ordersResponse = await ApiClient.getMyOrders(token);
      final orders = ordersResponse
          .map((json) => OrderDto.fromJson(json as Map<String, dynamic>))
          .toList();

      // Find order item that contains this ticket
      OrderItemDto? foundOrderItem;
      for (final order in orders) {
        for (final item in order.items) {
          if (item.tickets.any((t) => t.id == ticket.id)) {
      ...
      // Fetch event to get price tier name
